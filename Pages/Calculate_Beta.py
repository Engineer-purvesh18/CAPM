...
SP500 = SP500_raw[['Close']].reset_index()
SP500.columns = ['Date', 'sp500']

# downloading data for the stock
stocks_df = yf.download(stock, start=start, end=end)
stocks_df = stocks_df[['Close']].reset_index()
stocks_df.columns = ['Date', stock]
stocks_df['Date'] = pd.to_datetime(stocks_df['Date'].astype(str).str[:10])
SP500['Date'] = pd.to_datetime(SP500['Date'].astype(str).str[:10])
